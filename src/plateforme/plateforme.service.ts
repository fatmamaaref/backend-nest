...
export class PlateformeService {
  ...
  async linkBusinessToInstagram(oauthId: string, email: string, name: string, tokens: any, businessId: string) {
    try {
      console.log("🔍 Vérification de la plateforme Instagram avec oauthId:", oauthId);
      ...
      if (!plateforme) {
        ...
        console.log("🎉 Nouvelle plateforme Instagram créée :", plateforme);
      } else {
        console.log(`⚠️ Plateforme ${plateforme.id} existe déjà.`);
      }
      ...
      if (!existingBusinessPlatform) {
        console.log("✅ Création de l'association Business - Plateforme Instagram...");
      ...
      console.error("❌ Erreur dans linkBusinessToInstagram :", error);
      throw new BadRequestException("Erreur lors de l'ajout de la plateforme Instagram.");
    }
  }
